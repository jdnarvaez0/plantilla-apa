...
import { DocumentConfig, Author } from '../../common/interfaces/document-config.interface';
import { Reference } from '../../common/interfaces/reference.interface';

@Injectable()
export class ApaFormatterService {
  /**
  ...
  formatMultipleAuthors(authors: Author[]): string {
    ...
    if (authors.length === 2) {
      return `${this.formatAuthorName(authors[0])}, & ${this.formatAuthorName(authors[1])}`;
    }
    if (authors.length <= 20) {
      const allButLast = authors.slice(0, -1).map(a => this.formatAuthorName(a)).join(', ');
      const last = `& ${this.formatAuthorName(authors[authors.length - 1])}`;
      return `${allButLast}, ${last}`;
    }
    // 21+ autores: primeros 19 + ... + último
    const first19 = authors.slice(0, 19).map(a => this.formatAuthorName(a)).join(', ');
    const last = this.formatAuthorName(authors[authors.length - 1]);
    return `${first19}, ... ${last}`;
  }
  ...
  private formatBookReference(reference: Extract<Reference, { type: 'book' }>): string {
    const authors = this.formatMultipleAuthors(reference.authors);
    const year = `(${reference.year})`;
    const title = reference.title.endsWith('.') ? reference.title : `${reference.title}.`;
  ...
  private formatJournalReference(reference: Extract<Reference, { type: 'journal_article' }>): string {
    const authors = this.formatMultipleAuthors(reference.authors);
    const year = `(${reference.year})`;
    const title = reference.title.endsWith('.') ? reference.title : `${reference.title}.`;
  ...
  private formatWebsiteReference(reference: Extract<Reference, { type: 'website' }>): string {
    const authors = reference.authors.length > 0 
  ...
  private formatThesisReference(reference: Extract<Reference, { type: 'thesis' }>): string {
    const authors = this.formatMultipleAuthors(reference.authors);
    const year = `(${reference.year})`;
    const title = reference.title.endsWith('.') ? reference.title : `${reference.title}.`;
    const thesisType = reference.thesisType === 'doctoral' ? 'Tesis doctoral' : 'Tesis de maestría';
